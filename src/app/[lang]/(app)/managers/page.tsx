...
import type { ApprovalStatus, ManagerRole } from "@/lib/types";

export default async function ManagersPage(
  props: PageProps<"/[lang]/managers">,
) {
  ...
  const pendingCount = data.MANAGERS.filter(
    (m) => m.approvalStatus === "pending",
  ).length;
  const approvedCount = data.MANAGERS.filter(
    (m) => m.approvalStatus === "approved",
  ).length;
  const suspendedCount = data.MANAGERS.filter(
    (m) => m.approvalStatus === "suspended",
  ).length;
              ...
              <TableBody>
                {data.MANAGERS.map((m) => {
                  ...
                  return (
                    <TableRow key={m.id}>
                      <TableCell>
                        <Badge tone={APPROVAL_TONE[m.approvalStatus]}>
                          {approvalLabel}
                        </Badge>
                      </TableCell>
                      ...
                      <TableCell>
                        <span className="text-sm text-muted">{m.joinedAt}</span>
                      </TableCell>
                      <TableCell>
